...
{  
   var now= new Date();
    if( ((now-_count)/1000) < lifespan)
    {
          
        var per=Math.round(100 - ( (((now-_count)/1000)/lifespan) * 100 ) );
        if(per <= 20)
         $("#life_bar").html('<div  style="width: '+per+'%;" class="progress-bar progress-bar-danger">'+per+'%</div>');
        else
        $("#life_bar").html('<div  style="width: '+per+'%;" class="progress-bar progress-bar-info">'+per+'%</div>');
